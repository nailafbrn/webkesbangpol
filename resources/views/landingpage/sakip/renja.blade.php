...
@section('content')
    <section class="renja-hero">
        <div class="renja-overlay"></div>
        <div class="renja-content">
            <div class="hero-badge">
                <span class="badge-text">Work Plan</span>
            </div>
            <h1 class="renja-title">RENCANA KERJA</h1>
            <p class="renja-subtitle">Badan Kesatuan Bangsa dan Politik Kota Bandung</p>
            <p class="renja-lead">Dokumen perencanaan tahunan yang memuat program dan kegiatan yang diperlukan untuk mencapai sasaran pembangunan</p>
        </div>
    </section>
            ...
            <div class="filter-section">
                <div class="filter-header">
                    <div class="filter-title-wrapper">
                        ...
                        <p class="filter-subtitle">Temukan dokumen Rencana Kerja dengan mudah</p>
                    </div>
                </div>
                    ...
                    <div class="filter-row">
                        <div class="filter-item">
                            ...
                            <select class="filter-select" id="tahunFilter">
                                ...
                                @php
                                    $years = $renjas->pluck('tahun')->unique()->sort()->reverse();
                                @endphp
                    ...
                    <div class="table-container">
                        <table class="renja-table">
                            ...
                            <tbody id="renjaTableBody">
                                @forelse($renjas as $index => $renja)
                                <tr class="renja-item table-row" data-year="{{ $renja->tahun }}" data-title="{{ $renja->title }}" data-index="{{ $index + 1 }}">
                                    <td class="item-number table-cell-number">
                                        <span class="row-number">{{ $index + 1 }}</span>
                                    </td>
                                    <td class="table-cell-year">
                                        <span class="badge badge-year">{{ $renja->tahun }}</span>
                                    </td>
                                    <td class="table-cell-title">
                                        <div class="document-title">{{ $renja->title }}</div>
                                    </td>
...
<a href="{{ asset($renja->file_upload_wm) }}" class="btn btn-download" download>
        ...
        <div class="container">
            <div class="page-share p-3 rounded shadow-sm" style="background: #ffffff;">
                ...
                <div class="share-options d-flex gap-3">
                    ...
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->fullUrl()) }}&text={{ urlencode('Rencana Kerja Badan Kesatuan Bangsa dan Politik Kota Bandung') }}" 
                        target="_blank" class="share-icon twitter" title="Bagikan ke Twitter">
                        <i class="fab fa-x-twitter"></i>
                    </a>
                    <a href="https://api.whatsapp.com/send?text={{ urlencode('Rencana Kerja Badan Kesatuan Bangsa dan Politik Kota Bandung') }}%20{{ urlencode(request()->fullUrl()) }}" 
                        target="_blank" class="share-icon whatsapp" title="Bagikan ke WhatsApp">
                        <i class="fab fa-whatsapp"></i>
                    </a>
                    <a href="mailto:?subject={{ urlencode('Rencana Kerja Badan Kesatuan Bangsa dan Politik Kota Bandung') }}&body={{ urlencode('Saya ingin berbagi halaman menarik ini: ' . request()->fullUrl()) }}" 
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            const loadPdf = async (filename) => {
                ...
                try {
                    ...
                    try {
                        ...
                        const alternativeUrl = `/secure-file-renja/${encodeURIComponent(filename)}`;
                        ...
                        try {
                            ...
                            pdfContainer.innerHTML = `
                                <object 
                                    data="/secure-file-renja/${encodeURIComponent(filename)}" 
                                    ...
                                    <div class="text-center p-5">
                                        ...
                                        <a href="/secure-file-renja/${encodeURIComponent(filename)}" class="btn btn-primary" target="_blank">
            ...
            previewButtons.forEach(button => {
                button.addEventListener('click', async function () {
                    ...
                    if (downloadUrl) {
                        ...
                        downloadBtn.setAttribute('href', `/file-content-renja/${encodeURIComponent(filename)}`);
                    ...
                    try {
                        ...
                        const debugResponse = await fetch(`/debug-file-renja/${encodeURIComponent(filename)}`);
            ...
            const tableBody = document.getElementById('renjaTableBody');

            // Simpan semua baris asli sekali saja
            const allRows = Array.from(tableBody.querySelectorAll('tr.renja-item'));
