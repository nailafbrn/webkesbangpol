...
@section('content')
    <section class="iku-hero">
        <div class="iku-overlay"></div>
        <div class="iku-content">
            <div class="hero-badge">
                <span class="badge-text">Performance Indicators</span>
            </div>
            <h1 class="iku-title">INDIKATOR KINERJA UTAMA</h1>
            <p class="iku-subtitle">Badan Kesatuan Bangsa dan Politik Kota Bandung</p>
            <p class="iku-lead">Dokumen yang memuat ukuran kinerja utama sebagai acuan dalam mengukur keberhasilan pencapaian tujuan dan sasaran strategis instansi</p>
        </div>
    </section>
    ...
    <div class="main-container">
        <div class="content-wrapper">
            ...
            <div class="filter-section">
                <div class="filter-header">
                    <div class="filter-title-wrapper">
                        ...
                        <p class="filter-subtitle">Temukan dokumen Indikator Kinerja Utama dengan mudah</p>
                    </div>
                </div>
                    ...
                    <div class="filter-row">
                        <div class="filter-item">
                            ...
                            <select class="filter-select" id="tahunFilter">
                                ...
                                @php
                                    $years = $ikus->pluck('tahun')->unique()->sort()->reverse();
                                @endphp
                    ...
                    <div class="table-container">
                        <table class="iku-table">
                            ...
                            <tbody id="ikuTableBody">
                                ...
                                <tr class="iku-item table-row" data-year="{{ $iku->tahun }}" data-title="{{ $iku->title }}" data-index="{{ $index + 1 }}">
                                    <td class="item-number table-cell-number">
                                        <span class="row-number">{{ $index + 1 }}</span>
                                    </td>
                                    <td class="table-cell-year">
                                        <span class="badge badge-year">{{ $iku->tahun }}</span>
                                    </td>
                                    <td class="table-cell-title">
                                        <div class="document-title">{{ $iku->title }}</div>
                                    </td>
                                    <td class="table-cell-actions">
                                        <div class="action-buttons">
                                            <button type="button" class="btn btn-preview preview-btn" 
                                                    ...
                                                    data-file="{{ basename($iku->file_upload) }}" 
                                                    data-download-url="{{ asset($iku->file_upload_wm) }}"
                                                    data-title="{{ $iku->title }}">
                                                <i class="fas fa-eye"></i>
                                                <span class="btn-text">Preview</span>
                                            </button>
                                            <a href="{{ asset($iku->file_upload_wm) }}" class="btn btn-download" download>
            ...
            <div class="pagination-wrapper">
                {{ $ikus->links('pagination::bootstrap-5') }}
            </div>
        </div>
    </div>
        ...
        <div class="container">
            <div class="page-share p-3 rounded shadow-sm" style="background: #ffffff;">
                ...
                <div class="share-options d-flex gap-3">
                    ...
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->fullUrl()) }}&text={{ urlencode('Indikator Kinerja Utama Badan Kesatuan Bangsa dan Politik Kota Bandung') }}" 
                        target="_blank" class="share-icon twitter" title="Bagikan ke Twitter">
                        <i class="fab fa-x-twitter"></i>
                    </a>
                    <a href="https://api.whatsapp.com/send?text={{ urlencode('Indikator Kinerja Utama Badan Kesatuan Bangsa dan Politik Kota Bandung') }}%20{{ urlencode(request()->fullUrl()) }}" 
                        target="_blank" class="share-icon whatsapp" title="Bagikan ke WhatsApp">
                        <i class="fab fa-whatsapp"></i>
                    </a>
                    <a href="mailto:?subject={{ urlencode('Indikator Kinerja Utama Badan Kesatuan Bangsa dan Politik Kota Bandung') }}&body={{ urlencode('Saya ingin berbagi halaman menarik ini: ' . request()->fullUrl()) }}" 
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            const loadPdfFromBase64 = async (filename) => {
                try {
                    ...
                    const response = await fetch(`/serve-document-iku/${encodeURIComponent(filename)}`);
                    if (!response.ok) {
            ...
            const loadPdf = async (filename) => {
                ...
                try {
                    ...
                    try {
                        ...
                        const alternativeUrl = `/secure-file-iku/${encodeURIComponent(filename)}`;
                        ...
                        try {
                            ...
                            pdfContainer.innerHTML = `
                                <object 
                                    data="/secure-file-iku/${encodeURIComponent(filename)}" 
                                    ...
                                    <div class="text-center p-5">
                                        ...
                                        <a href="/secure-file-iku/${encodeURIComponent(filename)}" class="btn btn-primary" target="_blank">
                            ...
                            pdfContainer.innerHTML = `
                                <div class="text-center p-5">
                                    <div class="alert alert-warning">
                                        ...
                                        <a href="/file-content-iku/${encodeURIComponent(filename)}" class="btn btn-primary" download="${filename}">
            ...
            previewButtons.forEach(button => {
                button.addEventListener('click', async function () {
                    ...
                    if (downloadUrl) {
                        ...
                        downloadBtn.setAttribute('href', `/file-content-iku/${encodeURIComponent(filename)}`);
                    ...
                    try {
                        ...
                        const debugResponse = await fetch(`/debug-file-iku/${encodeURIComponent(filename)}`);
            ...
            const tableBody = document.getElementById('ikuTableBody');

            // Simpan semua baris asli sekali saja
            const allRows = Array.from(tableBody.querySelectorAll('tr.iku-item'));
