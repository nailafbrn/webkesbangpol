                ...
                <div class="card border-0 shadow-sm rounded">
                    <div class="card-body">
                        <form action="{{ route('lakip.update', $lakip->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label for="title-lakip" class="form-label">Title</label>
                                <input type="text" class="form-control" id="title-lakip" name="title" value="{{ old('title', $lakip->title) }}" required autocomplete="off">
                            </div>

                            <div class="mb-3">
                                <label for="tahun" class="form-label">Tahun</label>
                                <input type="text" class="form-control" id="tahun" name="tahun" value="{{ old('tahun', $lakip->tahun) }}" required autocomplete="off">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if($lakip->file_upload)
                                    <p class="mt-2">File saat ini: <a href="{{ asset($lakip->file_upload) }}" target="_blank">Lihat File</a></p>
                                @endif
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if($lakip->file_upload_wm)
                                    <p class="mt-2">File saat ini: <a href="{{ asset($lakip->file_upload_wm) }}" target="_blank">Lihat File</a></p>
                                @endif
                            </div>

                            <button type="submit" class="btn btn-md btn-primary">UPDATE</button>
                            <a href="{{ route('lakip.index') }}" class="btn btn-md btn-secondary">BATAL</a>
                        </form>
                    </div>
                </div>
            </div>
