...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ route('admin.pemilu.pilpres.create') }}" class="btn btn-primary">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($paslon->capres_foto && file_exists(public_path($paslon->capres_foto)))
                                        <img src="{{ asset($paslon->capres_foto) }}" alt="Foto Capres" class="img-thumbnail mb-1" style="height: 80px; object-fit: cover;">
                                    @endif

                                    @if($paslon->cawapres_foto && file_exists(public_path($paslon->cawapres_foto)))
                                        <img src="{{ asset($paslon->cawapres_foto) }}" alt="Foto Cawapres" class="img-thumbnail" style="height: 80px; object-fit: cover;">
                                    @endif
                                </td>
                                ...
                                <td>
                                    <a href="{{ route('admin.pemilu.pilpres.show', $paslon->id) }}" class="btn btn-sm btn-info" title="Detail"><i class="fas fa-eye"></i></a>
                                    <a href="{{ route('admin.pemilu.pilpres.edit', $paslon->id) }}" class="btn btn-sm btn-warning" title="Edit"><i class="fas fa-edit"></i></a>
                                    <form action="{{ route('admin.pemilu.pilpres.destroy', $paslon->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                        @csrf
                                        @method('DELETE')
